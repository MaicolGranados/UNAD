    ...
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                if (!User.Identity.IsAuthenticated)
                {
                    ...
                    if (HttpContext.Request.Cookies["Registro"] == "Exitoso")
                    {
                        homeVm.resultado = "RegistroExitoso";
                        //HttpContext.Response.Cookies.Delete("Registro");
                    }
                    else if (HttpContext.Request.Cookies["Registro"] == "Existente")
                    {
                        homeVm.resultado = "Existente";
                    }
                    else if (HttpContext.Request.Cookies["Registro"] == "Error")
                    {
                        homeVm.resultado = "Error";
                    }

                    if (HttpContext.Request.Cookies["PFE"] == "Entregado")
                    {
                        homeVm.resultado = "PlanFalimiarEntregado";
                    }
                    else if (HttpContext.Request.Cookies["PFE"] == "Error")
                    {
                        homeVm.resultado = "PlanFalimiarError";
                    }
                    else if (HttpContext.Request.Cookies["Registro"] == "Brigada")
                    {
                        homeVm.resultado = "Brigada";
                    }
                    else if (HttpContext.Request.Cookies["Pago"] == "Error")
                    {
                        homeVm.resultado = "ErrorPago";
                    }
                    else if (HttpContext.Request.Cookies["Pago"] == "Data")
                    {
        ...
        public async Task<IActionResult> RegistroEvento(int id)
        {
            try
            {
                ...
                var voluntarioid = HttpContext.Request.Cookies["IdVoluntario"];
                ...
                if (articulo.voluntario > 0)
                {
                    ...
                    if (articulo.Articulo == null)
                    {
                        ...
                        cookieOptions.Expires = new DateTimeOffset(DateTime.Now.AddSeconds(5));
                        HttpContext.Response.Cookies.Append("Pago", "Data", cookieOptions);
                        return RedirectToAction(nameof(Index));
                    }
                }
                else
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (articulovm.AsistenteSalida != null)
                    {
                        ...
                        var cookie1 = HttpContext.Request.Cookies["SessionId"];
                        if (cookie1 != null)
                        {
                            CookieOptions cookieOp = new CookieOptions();
                            cookieOp.Expires = new DateTimeOffset(DateTime.Now.AddMilliseconds(1));
                            HttpContext.Response.Cookies.Append("SessionId", encrypt.sha256(Convert.ToString("Temporal")), cookieOp);
                            Thread.Sleep(60);
                        }
                        var cookie2 = HttpContext.Request.Cookies["IdVoluntario"];
                        if (cookie2 != null)
                        {
                            CookieOptions cookieOp = new CookieOptions();
                            cookieOp.Expires = new DateTimeOffset(DateTime.Now.AddMilliseconds(1));
                            HttpContext.Response.Cookies.Append("IdVoluntario", encrypt.sha256(Convert.ToString("Temporal")), cookieOp);
                            Thread.Sleep(60);
                        }

                        CookieOptions cookieOptions = new CookieOptions();
                        cookieOptions.Expires = new DateTimeOffset(DateTime.Now.AddMinutes(30));
                        HttpContext.Response.Cookies.Append("IdVoluntario", asistenteData.Id.ToString(), cookieOptions);
                        HttpContext.Response.Cookies.Append("SessionId","S-" + encrypt.sha256(Convert.ToString(asistenteData.Id)), cookieOptions);
                        HttpContext.Response.Cookies.Append("ServiceId",Convert.ToString(servicioId.ServicioId), cookieOptions);
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (resultado == 0)
                    {
                        ...
                        HttpContext.Response.Cookies.Append("SessionId", encrypt.sha256(Convert.ToString(voluntario)), cookieOptions);
                        ...
                        if (estadoVoluntario.EstadoId == 5)
                        {
                            ...
                            HttpContext.Response.Cookies.Append("idService", Convert.ToString(1), cookieOptions);
                            HttpContext.Response.Cookies.Append("docVoluntario", documento, cookieOptions);
                        }
        ...
        public async Task<IActionResult> Registro(VoluntarioVM bri)
        {
            try
            {
                ...
                else
                {
                    CookieOptions cookieOptions = new CookieOptions();
                    cookieOptions.Expires = new DateTimeOffset(DateTime.Now.AddMinutes(10));
                    HttpContext.Response.Cookies.Append("IdColegio", bri.Colegio.Id.ToString(), cookieOptions);
                    HttpContext.Response.Cookies.Append("IdJornada", bri.JornadaColegio.Id.ToString(), cookieOptions);
        ...
        public async Task<IActionResult> RegistroParticipante(VoluntarioVM bri)
        {
            try
            {
                int brigada = Convert.ToInt32(HttpContext.Request.Cookies["IdColegio"]);
                int jornada = Convert.ToInt32(HttpContext.Request.Cookies["IdJornada"]);
                bri.Voluntario = new FUNSAR.Models.Voluntario();
        ...
        public async Task<IActionResult> Create(VoluntarioVM volvm)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    var colegio = _contenedorTrabajo.Colegio.GetFirstOrDefault(c => c.Id == Convert.ToInt32(HttpContext.Request.Cookies["IdColegio"]));
                    var jornada = _contenedorTrabajo.JornadaColegio.GetFirstOrDefault(j => j.Id == Convert.ToInt32(HttpContext.Request.Cookies["IdJornada"]));
                    if (colegio != null && jornada != null)
                    {
                        ...
                        {
                            if (HttpContext.Request.Cookies["IdAcudiente"] != null)
                            {
                                if (idVol == 0)
                                {
                                    ...
                                    acudiente.Id = Convert.ToInt32(HttpContext.Request.Cookies["IdAcudiente"]);
                                    acudiente.VoluntarioId = volvm.Voluntario.Id;
                                    _contenedorTrabajo.Acudiente.Update(acudiente);
                                    _contenedorTrabajo.save();
                                    CookieOptions cookieOptions = new CookieOptions();
                                    cookieOptions.Expires = new DateTimeOffset(DateTime.Now.AddSeconds(3));
                                    HttpContext.Response.Cookies.Append("Registro", "Exitoso", cookieOptions);
                                ...
                                else
                                {
                                    CookieOptions cookieOptions = new CookieOptions();
                                    cookieOptions.Expires = new DateTimeOffset(DateTime.Now.AddSeconds(3));
                                    HttpContext.Response.Cookies.Append("Registro", "Existente", cookieOptions);
                                    return RedirectToAction(nameof(Index));
                                }
                            }
                            if (idVol == 0)
                        ...
                        else
                        {
                            CookieOptions cookieOptions = new CookieOptions();
                            cookieOptions.Expires = new DateTimeOffset(DateTime.Now.AddSeconds(3));
                            HttpContext.Response.Cookies.Append("Registro", "Brigada", cookieOptions);
                            return RedirectToAction(nameof(Index));
                        }
                    }
                    else
                    {
                        CookieOptions cookieOptions = new CookieOptions();
                        cookieOptions.Expires = new DateTimeOffset(DateTime.Now.AddSeconds(3));
                        HttpContext.Response.Cookies.Append("Registro", "Error", cookieOptions);
                        return RedirectToAction(nameof(Index));
                    }
        ...
        public async Task<IActionResult> CreateA(VoluntarioVM volvm)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    HttpContext.Response.Cookies.Append("IdAcudiente", idA.ToString(), cookieOptions);
        ...
        public async Task<IActionResult> UploadPFE(IFormFile filePFE)
        {
            try
            {
                ...
                int id = Convert.ToInt32(HttpContext.Request.Cookies["IdVoluntario"]);
                ...
                if (filePFE is not null && volun.Voluntario is not null && filePFE.FileName.ToUpper().Contains(".PDF"))
                {
                    ...
                    CookieOptions cookieOptions = new CookieOptions();
                    cookieOptions.Expires = new DateTimeOffset(DateTime.Now.AddSeconds(5));
                    HttpContext.Response.Cookies.Append("PFE", "Entregado", cookieOptions);
                    return RedirectToAction(nameof(Index));
                }
                else
                {
                    CookieOptions cookieOptions = new CookieOptions();
                    cookieOptions.Expires = new DateTimeOffset(DateTime.Now.AddSeconds(5));
                    HttpContext.Response.Cookies.Append("PFE", "Error", cookieOptions);
                    return RedirectToAction(nameof(Index));
                }
        ...
        public async Task<IActionResult> GenerarPago(int id)
        {
            try
            {
                ...
                var sessionId = Convert.ToString(HttpContext.Request.Cookies["SessionId"]);
                var voluntario = Convert.ToString(HttpContext.Request.Cookies["IdVoluntario"]);
                if (sessionId != null)
                {
                ...
                else
                {
                    ...
                    else
                    {
                        CookieOptions cookieOptions = new CookieOptions();
                        cookieOptions.Expires = new DateTimeOffset(DateTime.Now.AddSeconds(5));
                        HttpContext.Response.Cookies.Append("Pago", "Error", cookieOptions);
                        return RedirectToAction(nameof(Index));
                    }
        ...
        public async Task<IActionResult> GenerarSesion(ArticuloVM articulo, string CategoriaId)
        {
            try
            {
                ...
                CookieOptions cookieOptions = new CookieOptions();
                cookieOptions.Expires = new DateTimeOffset(DateTime.Now.AddMinutes(30));
                HttpContext.Response.Cookies.Append("IdVoluntario", datoVoluntario.Id.ToString(), cookieOptions);
                HttpContext.Response.Cookies.Append("SessionId", encrypt.sha256(Convert.ToString(datoVoluntario.Id)), cookieOptions);
