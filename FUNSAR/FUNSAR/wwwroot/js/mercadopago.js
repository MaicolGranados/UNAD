    ...
    fetch(enviroment+'/Api/pagoServicios', {
        ...
        .then(data => {
            try {
                ...
                } else if (data.includes('NOK-V')){
                    document.getElementById('lblErrorValidate').textContent = 'No se obtuvo la información del voluntario, en caso de persistir informar al gestor de brigada.';
                }else {
...
function showDataPayment() {
    const url = enviroment + '/Api/DatosPago';
    ...
    fetch(url)
        .then(async function (response) {
            ...
            if (!response.ok) {
                throw new Error('Error en respuesta HTTP. Código: ' + response.status);
            }
            ...
            if (responseData.data != null) {
                ...
                if (responseData.data.detalle != null) {
                    ...
                    document.getElementById('lblErrorValidate').textContent = 'Ocurrió un error obteniendo los datos de pago. ID:1';
                    console.error('Detalle de pago no encontrado.');
                    return;
                }
            } else {
                document.getElementById('lblErrorValidate').textContent = 'Ocurrió un error obteniendo los datos de pago. ID:2';
                console.error('Datos de pago no encontrados.');
                return;
            }
        })
        .catch(function (reason) {
            document.getElementById('lblErrorValidate').textContent = 'Ocurrió un error obteniendo los datos de pago. ID:3';
            console.error('Error en fetch o parseo de datos:', reason);
        });
}
