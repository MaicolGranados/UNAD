    ...
    public class VoluntariosController : Controller
    {
        ...
        public async Task<IActionResult> GetAllxGestor()
        {
            try
            {
                ...
                else
                {
                    ...
                    //Opcion1
                return Json(new { data = _contenedorTrabajo.Voluntario.GetALL(filter: i => i.Colegio.BrigadaId == brigada && estado.Contains(i.estadoPersona.Id) && (i.FechaRegistro.Value >= ini && i.FechaRegistro.Value <= end), includeProperties: "Colegio,TDocumento,proceso,estadoPersona") });
            }
            catch (Exception ex)
            {
                HerramientasComunes.Log log = new HerramientasComunes.Log();
        ...
        public async Task<IActionResult> GetAllxDivision()
        {
            try
            {
                ...
                return Json(new { data = _contenedorTrabajo.Voluntario.GetALL(filter: i => division.Contains(i.Colegio.Brigada.Id), includeProperties: "Colegio,TDocumento,proceso,estadoPersona") });
