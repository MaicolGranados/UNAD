...
namespace FUNSAR.Areas.API
{
    ...
    public class ApiController : Controller
    {
        ...
        public async Task<ActionResult> pagoServicios([FromBody] servicioPago servicio)
        {
            try
            {
                ...
                if (detalle.Id == 1)
                {
                    ...
                    var asistencia = _contenedorTrabajo.Asistencia.GetALL(a => a.VoluntarioId == voluntario.Id);
                    if (asistencia.Count() > 0)
                    {
                        List<DateTime> fechaAsistencia = new List<DateTime>();
                        foreach (var item in asistencia)
                        {
                            fechaAsistencia.Add(item.Fecha);
                        }
                        DateTime fecha = fechaAsistencia.Max();
                        if ((fecha.Year > Convert.ToInt32(anio) && fecha.Month > Convert.ToInt32(mes)) || (fecha.Year == Convert.ToInt32(anio) && fecha.Month > Convert.ToInt32(mes)))
                        {
                                ...
                                switch (pagos.Estado)
                                {
                                    case "approved":
                                        if (pagos.ServicioId == 1)
                                        {
                                            ...
                                            _contenedorTrabajo.save();
                                            artivm.EstadoId = 7;
                                            _contenedorTrabajo.Voluntario.Update(artivm);
                                            _contenedorTrabajo.save();
        ...
        public async Task<ActionResult> DatosPago()
        {
            try
            {
                string id = Convert.ToString(HttpContext.Session.GetString("SessionId")) ?? "";
                var session = _contenedorTrabajo.Session.GetFirstOrDefault(filter: s => s.sessionid == id);
                if (session != null)
                {
                    ...
                    HttpContext.Session.SetString("servicioDetalle", dataPayment.Detalle);
                    return Json(new { data = dataPayment });
                }
                else
                {
                    HerramientasComunes.Log log = new HerramientasComunes.Log();
                    await log.escribirLog("ERROR", Convert.ToString(HttpContext.Session.GetString("SessionId"))?? "Es nulo sesion id", "HomeController.DatosPago()");
            ...
            {
                HerramientasComunes.Log log = new HerramientasComunes.Log();
                await log.escribirLog("ERROR", "SessionID" + Convert.ToString(HttpContext.Session.GetString("SessionId")) ?? "Es nulo sesion id" + ex.ToString(), "HomeController.DatosPago()");
        ...
        public async Task<IActionResult> GetCountPerson()
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    var voluntario = _contenedorTrabajo.Voluntario.GetALL(filter: v => v.Colegio.BrigadaId == usuario.BrigadaId && (v.FechaRegistro.Value >= ini && v.FechaRegistro.Value <= end),includeProperties: "Colegio");
